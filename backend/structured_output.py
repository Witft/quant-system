...
2. 【F-Score审计师】：基于上方给出的财务数据和皮奥特罗斯基F-Score理念，选出最关键的2个指标，列出当前值并说明是否令人担忧。优先从以下核心指标选取：经营现金流(每股)、净利润同比增长、销售毛利率、ROA、流动比率、速动比率、资产负债率。格式：「① 指标名：X → 说明… ② 指标名：X → 说明…」。如果细分指标大部分缺失（如银行股），请说明"该行业F-Score细分指标不适用"，改用ROE和负债率分析。
